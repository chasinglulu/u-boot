 ...
 * @NO{S02E02C14}
 * @ASIL{B}
 * @brief Get disk partition information
 *
				     ...
				     struct disk_partition *info)
{
	 ...
	 *
	 * <lba_start> <lba_size> [blkpart <num>]
	 */
	...
	if (raw_part_desc) {
		if (strcmp(strsep(&raw_part_desc, " "), "blkpart") == 0) {
			ulong blkpart = simple_strtoul(raw_part_desc, NULL, 0);
			int ret = blk_dselect_hwpart(dev_desc, blkpart);
			    ...
			    struct disk_partition *info)
{
	...
	devnum = env_get_ulong("fastboot.devnum", 10, 0);

	if (!ifname) {
		pr_err("boot device not found\n");
		return -ENODEV;
	}

	/* First try partition names on the default device */
	*dev_desc = blk_get_dev(ifname, devnum);
	if (*dev_desc) {
	...
	ret = part_get_info_by_dev_and_name_or_num(ifname, name, dev_desc,
						   info, true);
	return ret;
}
					  ...
					  struct disk_partition *info)
{
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Write/erase block device in chunks of FASTBOOT_MAX_BLK_WRITE
  *
  * @param[in] block_dev: Pointer to block device
  * @param[in] start: First block to write/erase
  * @param[in] blkcnt: Count of blocks
  * @param[in] buffer: Pointer to data buffer for write or NULL for erase
  *
  * @retval =0: success
  * @retval <0: failure
  *
  ...
  * @design
  */
static lbaint_t fb_generic_blk_write(struct blk_desc *block_dev, lbaint_t start,
 ...
 * @brief
 *
 * @param[in] info: Pointer to sparse storage
 * @param[in] blk: Pointer to block device
 * @param[in] blkcnt: Count of blocks
 * @param[in] buffer: Pointer to data buffer for write or NULL for erase
 *
 * @retval =0: success
 ...
 * @design
 */
static lbaint_t fb_generic_sparse_write(struct sparse_storage *info,
		lbaint_t blk, lbaint_t blkcnt, const void *buffer)
{
	struct fb_generic_sparse *sparse = info->priv;
	struct blk_desc *dev_desc = sparse->dev_desc;

	return fb_generic_blk_write(dev_desc, blk, blkcnt, buffer);
}

static lbaint_t fb_generic_sparse_reserve(struct sparse_storage *info,
 ...
 * @NO{S02E02C14}
 * @ASIL{B}
 * @brief Write raw image into block device
 *
 * @param[in] dev_desc:
 * @param[in] info: partition info
 * @param[in] part_name: partition name
 * @param[in] buffer: Pointer to image data
 * @param[in] download_bytes: Size of image data
 * @param[out] response: Pointer to fastboot response buffer
 *
 *
 * @data_read None
 * @data_read None
			    ...
			    void *buffer, u32 download_bytes, char *response)
{
	...
	blks = fb_generic_blk_write(dev_desc, info->start, blkcnt, buffer);
...
static int fb_generic_erase_mmc_hwpart(struct blk_desc *dev_desc)
{
	lbaint_t blks;

	debug("Start Erasing mmc hwpart[%u]...\n", dev_desc->hwpart);

	blks = fb_generic_blk_write(dev_desc, 0, dev_desc->lba, NULL);
...
static void fb_generic_boot_ops(struct blk_desc *dev_desc, void *buffer,
			    int hwpart, u32 buff_sz, char *response)
{
	...
	if (buffer) { /* flash */
		...
		blks = fb_generic_blk_write(dev_desc, 0, blkcnt, buffer);
		...
		if (fb_generic_erase_mmc_hwpart(dev_desc)) {
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Read Android boot image header from boot partition.
  *
  * @param[in/out] dev_desc: block device descriptor
  * @param[in/out] info: Boot partition info
  * @param[in/out] hdr: Where to store read boot image header
  ...
  * @design
  */
static lbaint_t fb_generic_get_boot_header(struct blk_desc *dev_desc,
				       struct disk_partition *info,
				       ...
				       char *response)
{
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Write downloaded zImage to boot partition and repack it properly.
  *
  * @param[in] dev_desc: block device descriptor
  * @param[in] download_buffer: Address to fastboot buffer with zImage in it
  * @param[in] download_bytes: Size of fastboot buffer, in bytes
  * @param[out] response: Pointer to fastboot response buffer
  *
  * @retval =0: success
  * @retval <0: failure
  ...
  * @design
  */
static int fb_generic_update_zimage(struct blk_desc *dev_desc,
				...
				char *response)
{
	...
	hdr_sectors = fb_generic_get_boot_header(dev_desc, &info, hdr, response);
	if (hdr_sectors == 0) {
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Lookup block device partion by name
  *
  * @param[in] part_name: Named partition to lookup
  * @param[out] dev_desc: Pointer to returned blk_desc pointer
  * @param[out] part_info: Pointer to returned struct disk_partition
  * @param[out] response: Pointer to fastboot response buffer
  *
  * @retval =0: success
  * @retval <0: failure
 ...
 * @NO{S02E02C14}
 * @ASIL{B}
 * @brief Get block device descriptor
 *
 * @param[out] response: Pointer to fastboot response buffer
 *
 * @retval =0: success
 ...
 * @design
 */
static struct blk_desc *fastboot_generic_get_dev(char *response)
{
	char *ifname;
	int devnum;
	...
	block_dev = blk_get_dev(ifname, devnum);
	if (!block_dev || block_dev->type == DEV_TYPE_UNKNOWN) {
		pr_err("invalid block device for %s:%d\n", ifname, devnum);
		fastboot_fail("invalid block device", response);
		return NULL;
	}
	return block_dev;
}
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Write image to block device for fastboot
  *
  * @param[in] cmd: Named partition to write image to
  * @param[in] download_buffer: Pointer to image data
  * @param[in] download_bytes: Size of image data
  * @param[out] response: Pointer to fastboot response buffer
  *
  *
			      ...
			      u32 download_bytes, char *response)
{
	...
	if (strcmp(cmd, CONFIG_FASTBOOT_MMC_BOOT1_NAME) == 0) {
		dev_desc = fastboot_generic_get_dev(response);
		if (dev_desc)
			fb_generic_boot_ops(dev_desc, download_buffer, 1,
	...
	if (strcmp(cmd, CONFIG_FASTBOOT_MMC_BOOT2_NAME) == 0) {
		dev_desc = fastboot_generic_get_dev(response);
		if (dev_desc)
			fb_generic_boot_ops(dev_desc, download_buffer, 2,
					download_bytes, response);
		return;
	}
#endif
	...
	if (strncasecmp(cmd, "zimage", 6) == 0) {
		dev_desc = fastboot_generic_get_dev(response);
		if (dev_desc)
			fb_generic_update_zimage(dev_desc, download_buffer,
					     download_bytes, response);
		return;
	}
#endif
	...
	if (!info.name[0] &&
	    fastboot_generic_get_part_info(cmd, &dev_desc, &info, response) < 0)
		return;

	if (is_sparse_image(download_buffer)) {
		struct fb_generic_sparse sparse_priv;
		...
		sparse.write = fb_generic_sparse_write;
		sparse.reserve = fb_generic_sparse_reserve;
  ...
  * @NO{S02E02C14}
  * @ASIL{B}
  * @brief Erase block device for fastboot
  *
  * @param[in] cmd: Named partition to erase
  * @param[out] response: Pointer to fastboot response buffer
  *
  *
  * @data_read None
...
void fastboot_generic_erase(const char *cmd, char *response)
{
	...
	if (strcmp(cmd, CONFIG_FASTBOOT_MMC_BOOT1_NAME) == 0) {
		...
		dev_desc = fastboot_generic_get_dev(response);
		if (dev_desc)
			fb_generic_boot_ops(dev_desc, NULL, 1, 0, response);
		return;
	}
	if (strcmp(cmd, CONFIG_FASTBOOT_MMC_BOOT2_NAME) == 0) {
		/* erase EMMC boot2 */
		dev_desc = fastboot_generic_get_dev(response);
		if (dev_desc)
			fb_generic_boot_ops(dev_desc, NULL, 2, 0, response);
		return;
	}
#endif
	...
	if (fastboot_generic_get_part_info(cmd, &dev_desc, &info, response) < 0)
		return;
	...
	blks = fb_generic_blk_write(dev_desc, blks_start, blks_size, NULL);
