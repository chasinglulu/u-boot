...
#include <dm/devres.h>
#include <errno.h>
#include <boot-device/bootdevice-gpio.h>
#include <boot-device/bootdevice.h>

DECLARE_GLOBAL_DATA_PTR;

static int boot_device_get(struct udevice *dev, u32 *buf)
{
	int ret;
	struct boot_device_gpio_platdata *plat_data;
...
static int boot_device_probe(struct udevice *dev)
{
	struct boot_device_gpio_platdata *plat_data;
...
static int boot_device_remove(struct udevice *dev)
{
	struct boot_device_gpio_platdata *plat_data;
...
#if CONFIG_IS_ENABLED(OF_CONTROL)
static const struct udevice_id boot_device_ids[] = {
...
U_BOOT_DRIVER(boot_device_gpio) = {
	.name = "boot-device-gpio",
	.id = UCLASS_BOOT_DEVICE,
	.probe = boot_device_probe,
	.remove = boot_device_remove,
#if CONFIG_IS_ENABLED(OF_CONTROL)
	.of_match = boot_device_ids,
#endif
	.plat_auto = sizeof(struct boot_device_gpio_platdata),
